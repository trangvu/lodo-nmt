...
class MultilingualDatasetManager(object):
    def __init__(self, args, lang_pairs, langs, dicts, sampling_method):
        ...
        self.dicts = dicts
        langtoks = []
        src_langtok_spec, tgt_langtok_spec = args.langtoks['main']
        for lang in langs:
            if src_langtok_spec is None:
                src_langtok_spec = 'src'
            langtok = get_lang_tok(
                lang=lang, lang_tok_style=args.lang_tok_style, spec=src_langtok_spec
            )
            langtoks.append(langtok)
        self.lang_dict = self.create_lang_dictionary(langtoks)
    ...
    def load_a_dataset(
        ...
        **extra_kwargs,
        ...
        logger.info(
            f"{data_category}:{src}-{tgt} src_langtok: {src_langtok} {src_dict[src_langtok] if src_langtok else src_langtok}; "
            f"tgt_langtok: "
            f"{tgt_langtok} {tgt_dict[tgt_langtok] if tgt_langtok else tgt_langtok}"
        )
