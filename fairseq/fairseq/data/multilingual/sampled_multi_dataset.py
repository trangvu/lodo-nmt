...
class SampledMultiDataset(FairseqDataset):
    ...
    def ordered_indices_of_single_dataset(self, dataset_index):
        if dataset_index == 0:
            indices = np.arange(self.cumulated_sizes[0])
        else:
            indices = np.arange(self.cumulated_sizes[dataset_index-1], self.cumulated_sizes[dataset_index])
        if self.shuffle:
