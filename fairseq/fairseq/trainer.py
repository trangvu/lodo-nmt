...
class Trainer(object):
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        logging_outputs, sample_size, ooms, drained_input = [], 0, 0, False
        for i, sample in enumerate(samples):  # delayed update loop
            if sample is not None and 'net_input' not in sample:
                drained_input = True
        ...
        if logging_output is not None:
            logging_output['drained_input'] = drained_input  # empty samples from dataloader
    ...
    def _prepare_sample(self, sample, is_dummy=False):
        ...
        if sample is None or len(sample) == 0 or ('net_input' not in sample):
            assert (
